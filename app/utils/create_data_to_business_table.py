...
path = "/home/czd-2019/Datasets/Models_pic_dataset/business_model/"
business_names=os.listdir(path)
...
for b_name in business_names:
    temp_path = os.path.join(path,b_name)
    pic_names=os.listdir(temp_path)
    ...
    for p_name in pic_names:
        FA_results = {}
        pic_path = os.path.join(temp_path,p_name)
        print(pic_path)
        ...
        if len(face_locations) == 0:
            ...
            if len(face_locations) == 0:
                print("{%s}. No face detected." % (pic_path))
                continue
            elif len(face_locations) > 1:
                print("{%s}. More than 1 face detected." % (pic_path))
                continue
        elif len(face_locations) > 1:
            print("{%s}. More than 1 face detected." % (pic_path))
            continue
        ...
        total_names.append(p_name.replace('.jpg',''))
        total_pic_paths.append(pic_path)
        ...
        # print(total_encodings)
        # print(total_names)
        # print(total_FA_results)

        # exit()

    cursor = conn.cursor()
    for [fa,fd],p in zip(list(zip(total_FA_results,total_encodings)),total_pic_paths):
        ...
        t1 = time.time()
        sql = """
        INSERT INTO Tbl_business (business_name,face_encodings,attribute_encodings,pic_path) VALUES(%s,%s,%s,%s)
        """
        cursor.execute(sql, (b_name, fd_json, fa_json,p))
        conn.commit()
        t2 = time.time()
        print("cost:{%.3f}"%(t2-t1))
